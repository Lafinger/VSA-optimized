...
int find_next_first(HalfedgeDS& he, int edge, int r, const MatrixXi& R) {
...
int find_next_second(HalfedgeDS& he, int edge, int r, const MatrixXi& R) {
    edge = he.getNext(edge);
    while (R(he.getFace(he.getOpposite(edge)),0)!=r || R(he.getFace(edge),0)==r) {
        edge = he.getNext(he.getOpposite(edge));
    }
...
vector<int> find_interior_neighbors(HalfedgeDS& he, int v, int r, const MatrixXi& R){
...
double get_length_edge(HalfedgeDS& he, int edge, const MatrixXd& V){
    Vector3d v1 = V.row(he.getTarget(edge));
    Vector3d v2 = V.row(he.getTarget(he.getOpposite(edge)));
    return (v2-v1).norm();
};

int find_englobing_region(int region, const MatrixXi& Regions, HalfedgeDS& he, const MatrixXi& Faces, const MatrixXd& Vertices){

    int englobing_region;

    MatrixXi adjacency = face_adjacency(Faces, Vertices.rows());

    int adj_face1;
    int adj_face2;
    int adj_face3;
    for (int f = 0; f < Regions.rows(); ++f){
        if(Regions(f,0)== region){
            
            adj_face1 = adjacency(f,0);
            adj_face2 = adjacency(f,1);
            adj_face3 = adjacency(f,2);
            if (Regions(adj_face1,0) != region){
                return Regions(adj_face1,0);
            }
            else if (Regions(adj_face2,0) != region){
                return Regions(adj_face2,0);
            }
            else if (Regions(adj_face3,0) != region){
                return Regions(adj_face3,0);
            }
        }
    }
...
MatrixXi update_englobing_region(const MatrixXi& R_bis,int region,int englobing_region){
...
vector<Vector3i> triangulate_region (const MatrixXi& Regions, int region, const vector<vector<int>>& Anchors, const MatrixXd& Vertices, const MatrixXi& Faces, HalfedgeDS& he){

    vector<Vector3i> triangles;
    // cout<<"\n\nregion "<<region<<"\n"<<endl;
    // cout<<"colors :\n"<<endl;
    MatrixXi color_graph = color_region(Regions, region, Anchors, Vertices, he);
    //for (int i=0 ; i < color_graph.size() ; i++){
    //    if (color_graph(i,0) != -1){
    //        // cout<<"vertex "<<i<<" color "<<color_graph(i,0)<<endl;
    //    }
    //}
    MatrixXi correspondence_color_anchor(Anchors[region].size(), 1);

    int anchor_vertex;
    int color;
    // cout<<"\nanchor vertices : "<<endl;
    for (int i=0 ; i < Anchors[region].size(); i++){
        anchor_vertex = Anchors[region][i];
    ...
    for (int f=0 ; f< Regions.size() ; f++){
        if (Regions(f,0)==region){
            color1 = color_graph(Faces(f,0), 0);
            color2 = color_graph(Faces(f,1), 0);
            color3 = color_graph(Faces(f,2), 0);
...
pair<MatrixXi,MatrixXi> triangulation (MatrixXi& Regions, const vector<vector<int>>& Anchors, const MatrixXd& Vertices, const MatrixXi& Faces, HalfedgeDS& he){
    ...
    int englobing_region;
    MatrixXi R_bis;
    for (int r = 0; r < Anchors.size(); ++r){
        if (Anchors[r].size() == 0){
            englobing_region = find_englobing_region(r, Regions, he, Faces, Vertices);
            R_bis = update_englobing_region(Regions, r, englobing_region);
            Regions = R_bis;
        }
    }
    ...
    for (int r = 0; r < Anchors.size(); ++r){
        ...
        if (Anchors[r].size()>=3){
            ...
            triangulation_region = triangulate_region(Regions, r, Anchors, Vertices, Faces, he);
